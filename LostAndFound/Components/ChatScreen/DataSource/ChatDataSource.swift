    ...
    func addMessages(_ messages: [QBChatMessage]) {
        serialQueue.async { [weak self] in
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                self.delegate?.chatDataSource(self, changeWithMessages: messagesArray, action: .add)
            }
        }
    }
    ...
    func deleteMessages(_ messages: [QBChatMessage]) {
        serialQueue.async { [weak self] in
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                self.delegate?.chatDataSource(self, changeWithMessages: messagesArray, action: .remove)
            }
        }
    }
    ...
    func updateMessages(_ messages: [QBChatMessage]) {
        serialQueue.async { [weak self] in
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                self.delegate?.chatDataSource(self, willChangeWithMessageIDs: IDs)
                self.delegate?.chatDataSource(self, changeWithMessages: messagesArray, action: .update)
            }
        }
    }
