...
struct ChatManagerConstant {
    ...
    static let notFound = "Dialog Removed"
}
...
class ChatManager: NSObject {
    ...
    func updateStorage() {
        ...
        if QBChat.instance.isConnected == false {
            self.delegate?.chatManager(self, didFailUpdateStorage: "Network Error")
            return
        }
        ...
        loadGroup.enter()
        updateAllDialogs(withPageLimit: 500,
    ...
    func createGroupDialog(withName name: String,
        ...
        QBRequest.createDialog(chatDialog, successBlock: { response, dialog in
            dialog.join(completionBlock: { (error) in
                ...
                self.delegate?.chatManager(self, didUpdateStorage: "Create New" + dialogName)
    ...
    func createPrivateDialog(withOpponent opponent: QBUUser, completion: DialogCompletion? = nil) {
