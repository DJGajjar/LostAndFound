...
class CallViewController: UIViewController, UICollectionViewDelegateFlowLayout {
    ...
    @IBOutlet private weak var ToolCallBar: ToolBar!

    //MARK:- Properties
    var CallToQuser = QBUUser()
    ...
    private lazy var statsItem = UIBarButtonItem(title: "Stats", style: .plain, target: self, action: #selector(updateStatsView))
    ...
    func configureGUI() {
        ...
        if let conferenceType = session?.conferenceType {
            ...
            case .video:
                ToolCallBar.add(ButtonsFactory.videoEnable(), action: { [weak self] sender in
                    if let muteVideo = self?.muteVideo {
                        self?.muteVideo = !muteVideo
                        self?.localVideoView?.isHidden = !muteVideo
                    }
                })
                ToolCallBar.add(ButtonsFactory.screenShare(), action: { [weak self] sender in
            ...
            case .audio:
                if UIDevice.current.userInterfaceIdiom == .phone {
                    ...
                    ToolCallBar.add(dynamicButton, action: { sender in
            ...
            ToolCallBar.add(audioEnabled, action: { [weak self] sender in
            ...
            ToolCallBar.add(ButtonsFactory.decline(), action: { [weak self] sender in
                self?.session?.hangUp(["hangup": "hang up"])
            })
        }

        ToolCallBar.updateItems()
        let mask: UIView.AutoresizingMask = [.flexibleWidth, .flexibleHeight, .flexibleLeftMargin, .flexibleRightMargin, .flexibleTopMargin, .flexibleBottomMargin]
    ...
    func startCall() {
        ...
        beepTimer = Timer.scheduledTimer(timeInterval: QBRTCConfig.dialingTimeInterval(), target: self, selector: #selector(playCallingSound(_:)), userInfo: nil, repeats: true)
    ...
    private func closeCall() {
        ...
        ToolCallBar.isUserInteractionEnabled = false
        UIView.animate(withDuration: 0.5) {
            self.ToolCallBar.alpha = 0.4
        }
        state = .disconnected
    ...
    private func userView(userID: UInt) -> UIView? {
        let profile = Profile()
        if profile.isFull == true, profile.ID == userID, session?.conferenceType != .audio {
            if cameraCapture?.hasStarted == false {
                cameraCapture?.startSession(nil)
                session?.localMediaStream.videoTrack.videoCapture = cameraCapture
            }
    ...
    private func createConferenceUser(userID: UInt) -> User {
        return User(user: self.CallToQuser)
    }
...
extension CallViewController: QBRTCClientDelegate {
    ...
    func session(_ session: QBRTCBaseSession, updatedStatsReport report: QBRTCStatsReport, forUserID userID: NSNumber) {
        ...
        if user.connectionState == .connected, report.videoReceivedBitrateTracker.bitrate > 0.0 {
    ...
    func session(_ session: QBRTCBaseSession, connectedToUser userID: NSNumber) {
        ...
        if callTimer == nil {
            ...
            callTimer = Timer.scheduledTimer(timeInterval: CallConstant.refreshTimeInterval, target: self, selector: #selector(refreshCallTime(_:)), userInfo: nil, repeats: true)
        }
    }
...
extension CallViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CallConstant.opponentCollectionViewCellIdentifier, for: indexPath) as? UserCell
        else {
