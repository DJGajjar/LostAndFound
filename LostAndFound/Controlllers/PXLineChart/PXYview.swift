...
class PXYview: UIView {
    ...
    fileprivate var perPixelOfYvalue: Float = -1
    ...
    func reloadYaxis() {
        ...
        perPixelOfYvalue = -1;
        ylineView.frame = CGRect(x: self.frame.width-1, y: 0, width: 0, height: 10)
        ...
        for index in 0..<elementCons {
            if let elementView = delegate?.elementWithAxisType(.AxisTypeY, index) {
                let attr = [NSAttributedString.Key.font: elementView.font!]
                ...
                elementView.frame = CGRect(x: 0, y: self.frame.height-((elementSize.height/2)+y), width: self.frame.width-5, height: 10)
