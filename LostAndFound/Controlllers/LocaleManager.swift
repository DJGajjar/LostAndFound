...
public class LocaleManager: NSObject {
    ...
    */
    internal class func reloadWindows(animated: Bool = true) {
        ...
        if animated {
            windows.first.map {
                UIView.transition(with: $0, duration: 0.01, options: .transitionCrossDissolve, animations: nil, completion: nil)
            }
        }
    }
...
public extension UITextField {
    ...
    var originalAlignment: NSTextAlignment? {
        ...
        set {
            if let newValue = newValue {
                objc_setAssociatedObject(self, &AssociatedKeys.originalAlignment, newValue.rawValue as NSNumber, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
            }
        }
    }
    ...
    var forcedAlignment: NSTextAlignment? {
        ...
        set {
            if let newValue = newValue {
                objc_setAssociatedObject(self, &AssociatedKeys.forcedAlignment, newValue.rawValue as NSNumber, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
            }
        }
    }
