...
class AboutUs: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        apiClass().doNormalAPI(param: [:], APIName: apiClass().AboutUsAPI, method: "GET") { (success, errMessage, mDict) in
            DispatchQueue.main.async {
                if success == true {
                    self.txtContent.attributedText = constants().setHTMLFromString(text: (mDict.value(forKey: "about_us") as! NSDictionary).value(forKey: "description") as! String)
                }
            }
        }
    }
    ...
    //MARK:- Other Methods
    func doSetFrames() {
        self.lblTitle.text = NSLocalizedString("aboutus", comment: "")
